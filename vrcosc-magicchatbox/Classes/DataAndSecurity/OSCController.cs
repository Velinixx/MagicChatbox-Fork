...
public static class OSCController
{
    ...
    public static void AddC20HeartRate(List<string> Uncomplete)
    {
        if (ViewModel.Instance.IntgrC20HeartRate)
        {
            string output = ViewModel.Instance.C20HeartRateModule.GetHeartRateString();
            if (string.IsNullOrEmpty(output))
            {
                return;
            }
            TryAddToUncomplete(Uncomplete, output, "C20HeartRate");
        }
    }
    ...
    public static void BuildOSC()
    {
        ...
        var functionMap = new Dictionary<Func<bool>, Action<List<string>>>
        {
            ...
            {
                () => ViewModel.Instance.IntgrC20HeartRate_VR &&
                ViewModel.Instance.IsVRRunning ||
                ViewModel.Instance.IntgrC20HeartRate_DESKTOP &&
                !ViewModel.Instance.IsVRRunning,
                AddC20HeartRate
            },
        };
    ...
    public static void SetOpacity(string controlName, string opacity)
    {
        switch (controlName)
        {
            ...
            case "MediaLink":
                ...
                break;
            case "C20HeartRate":
                ViewModel.Instance.C20HeartRate_Opacity = opacity;
                break;
            default:
                break;
        }
